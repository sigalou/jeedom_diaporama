...
{
	$icon = $('.eqLogicAttr[data-l1key=configuration][data-l2key=cheminDiaporamaValide]').value();
	if($icon != '' && $icon != null)
		$('#img_device').attr("src", 'plugins/diaporama/desktop/images/' + $icon + '.png');
});
...
$('.eqLogicAttr[data-l1key=configuration][data-l2key=localEtat]').on('change', function ()
{
	...
	if($icon != '' && $icon != null)
		$('#img_local').attr("src", 'plugins/diaporama/desktop/images/jeedom_' + $icon + '.png');
});
...
$("#bt_addespeasyAction").off('click').on('click', function(event)
{
 ...
 $('#bt_addEvent').off('click').on('click', function () {
    ...
    $('#md_modal').load('index.php?v=d&plugin=diaporama&modal=alarm&eqLogic_id=' + $('.eqLogicAttr[data-l1key=id]').value()).dialog('open');
});

$('#bt_media').off('click').on('click', function ()
{
  $('#md_modal').dialog({title: "{{Info Media}}"});
  $('#md_modal').load('index.php?v=d&plugin=diaporama&modal=media&iddevice='+ $('.eqLogicAttr[data-l1key=logicalId]').value()).dialog('open');
});
$('#bt_req').off('click').on('click', function ()
{
  $('#md_modal').dialog({title: "{{Requêteur JSON}}"});
  $('#md_modal').load('index.php?v=d&plugin=diaporama&modal=req&iddevice='+ $('.eqLogicAttr[data-l1key=logicalId]').value()).dialog('open');
});
$('#bt_req2').off('click').on('click', function ()
{
  $('#md_modal').dialog({title: "{{Requêteur}}"});
  $('#md_modal').load('index.php?v=d&plugin=diaporama&modal=req2&iddevice='+ $('.eqLogicAttr[data-l1key=logicalId]').value()).dialog('open');
});
$('#bt_sante').off('click').on('click', function ()
{
  $('#md_modal').dialog({title: "{{Liste Amazon Echo}}"});
  $('#md_modal').load('index.php?v=d&plugin=diaporama&modal=health').dialog('open');
});

$('#bt_reminders').off('click').on('click', function ()
{
  $('#md_modal').dialog({title: "{{Rappels/Alarmes}}"});
  $('#md_modal').load('index.php?v=d&plugin=diaporama&modal=reminders').dialog('open');
});

$('#bt_history').off('click').on('click', function ()
{
  $('#md_modal').dialog({title: "{{Historique}}"});
  $('#md_modal').load('index.php?v=d&plugin=diaporama&modal=history').dialog('open');
});

$('#bt_routines').off('click').on('click', function ()
{
  $('#md_modal').dialog({title: "{{Routines}}"});
  $('#md_modal').load('index.php?v=d&plugin=diaporama&modal=routines').dialog('open');
});
...
{
	// Ici il faudra faire comme dans desktop/php/diaporama.php ligne 268 chercher type et à défaut family
	$icon = $('.eqLogicAttr[data-l1key=configuration][data-l2key=type]').value();
	if($icon != '' && $icon != null)
		$('#img_device').attr("src", 'plugins/diaporama/core/config/devices/' + $icon + '.png');
...
jeedom.plugin.getDeamonCookieInfo = function(_params)
{
    ...
    paramsAJAX.url = 'plugins/diaporama/core/ajax/diaporama.ajax.php';
    paramsAJAX.data = {
...
jeedom.plugin.VerifiePresenceCookie = function(_params)
{
 ...
 jeedom.plugin.node.action = function (_params) { //Delete reminder
 	...
 	var params = $.extend({}, jeedom.private.default_params, paramsSpecifics, _params || {});
 	var paramsAJAX = jeedom.private.getParamsAJAX(params);
 	paramsAJAX.url = 'plugins/diaporama/desktop/php/diaporamaProxy.php';
 	paramsAJAX.data = {

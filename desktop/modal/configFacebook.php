...
echo '<form name="formulaire" id="div_eventEditAlert" class="form-horizontal"><fieldset>';
	...
	if (is_object($device)) {
		...
		//log::add('diaporama', 'debug', 'Récupération de : '.json_encode($Albums).' depuis plugin/device('.$_GET['iddevice'].')/config/arrayAlbumsFacebook');
		foreach (config::byKey('albumsFacebook', 'diaporama', '0') as $value) {
			$checked="";
